...
class TP_agent(Agent):
    ...
    def go(self, n):
        ...
        """
        for i in range(n):
            if self.last_price < 0.4*self.ave and self.instock < 60:
                tobuy = 40
...
class Plot_prices(object):
    ...
    def plot_run(self):
        ...
        plt.plot(range(num),env.stock_history,label="In stock when last price is(.4)")
        plt.plot(range(num),env.price_history,label="Price when last price is(.4)")
