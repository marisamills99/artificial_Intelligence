...
class Rob_top_layer(Environment):
    def __init__(self, middle, timeout=500, locations = {'mail':(-5,10), 
...
trap_top = Rob_top_layer(trap_middle,locations={'goal':goal[i]})
...
#when do you return from middle?
while arrived== {'arrived':'Interupt'} and i<3:
    i+=1
    trap_top = Rob_top_layer(trap_middle,locations={'goal':goal[i]})
    pl=Plot_env(trap_body,trap_top)
    trap_top.do({'visit':['goal']})
